...
class DataManager:
    ...
    def insert_user_customization(self, name, email, prefecture, baseid, colorids, interiorids, exteriorids):
        ...
        user_id = _db_manager.insert_record("Users", {"username":name, "email":email, "place":prefecture})
        customization_id = _db_manager.insert_record("Customizations", {"userid":self._to_int(user_id), "baseid":self._to_int(baseid)})
        for colorid in colorids:
            _db_manager.insert_record("ColorCustomizations", {"customizationid":customization_id, "colorid":colorid})
        for interiorid in interiorids:
            _db_manager.insert_record("InteriorCustomizations", {"customizationid":customization_id, "interiorid":interiorid})
        for exteriorid in exteriorids:
            _db_manager.insert_record("ExteriorCustomizations", {"customizationid":customization_id, "exteriorid":exteriorid})
