...
- さらに、以下3点を重視して詳細の設計を行っています。
    ...
    3. ドメイン特化型のリソースはページ開発用のインターフェースとして組み合わせて活用することを意図して設計しました。
    4. 将来的に他のフレームワークに移行する可能性も考え、streamlitに依存する範囲を限定するべく、バックエンドのlogicをdata_manage.pyに移行しようとしましたが、途中で力つきました。

※参考：Streamlitに依存する範囲
![image](image/class_design_streamlit.PNG)
